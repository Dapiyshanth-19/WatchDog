# -*- coding: utf-8 -*-
"""
WatchDog – Core video processing pipeline (v4 – Game + Face Edition).

Frame flow:
    capture → resize → YOLO detect → SORT track → behaviour analyse
    → face recognition (async) → game logic → draw boxes → vision FX → stream
"""
...
def _draw_boxes(frame, track_data, player_snapshot: dict | None = None,
    ...
    for td in track_data:
        ...
        if is_eliminated:
            cv2.putText(frame, "ELIMINATED", (x1, y2 + 16),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.52, _ELIM_COLOR, 2, cv2.LINE_AA)
        elif is_winner:
            cv2.putText(frame, "WINNER!", (x1, y2 + 16),
...
def _run(initial_source):
    ...
    model_path = _cfg.get("model_path", MODEL_NAME)
    try:
    ...
    while not _stop_event.is_set():
        ...
        new_path = _cfg.get("model_path", MODEL_NAME)
